...
from examples.configs.rpdg_configs import RPDG_DEFAULT_CONFIG, RPDGTrainConfig
from osrl.algorithms import RPDG, RPDGTrainer
from osrl.common import TransitionDataset
from osrl.common.exp_util import auto_name, seed_all


@pyrallis.wrap()
def train(args: RPDGTrainConfig):
    # update config: merge CLI overrides into task-specific defaults
    cfg, old_cfg = asdict(args), asdict(RPDGTrainConfig())
    differing_values = {key: cfg[key] for key in cfg.keys() if cfg[key] != old_cfg[key]}
    cfg = asdict(RPDG_DEFAULT_CONFIG[args.task]())
    cfg.update(differing_values)
    args = types.SimpleNamespace(**cfg)

    # setup logger
    default_cfg = asdict(RPDG_DEFAULT_CONFIG[args.task]())
