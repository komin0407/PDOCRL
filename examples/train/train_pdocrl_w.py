...
from examples.configs.pdocrl_w_configs import PDOCRL_W_DEFAULT_CONFIG, PDOCRL_WTrainConfig
...
@pyrallis.wrap()
def train(args: PDOCRL_WTrainConfig):
    # update config: merge CLI overrides into task-specific defaults
    cfg, old_cfg = asdict(args), asdict(PDOCRL_WTrainConfig())
    differing_values = {key: cfg[key] for key in cfg.keys() if cfg[key] != old_cfg[key]}
    cfg = asdict(PDOCRL_W_DEFAULT_CONFIG[args.task]())
    cfg.update(differing_values)
    args = types.SimpleNamespace(**cfg)

    # setup logger
    default_cfg = asdict(PDOCRL_W_DEFAULT_CONFIG[args.task]())
    ...
    if "Metadrive" in args.task:
        import gym as gym_metadrive
        env = gym_metadrive.make(args.task)
    else:
    ...
    model = PDOCRL_W(
        ...
        v_hidden_sizes=args.v_hidden_sizes,
        w_hidden_sizes=args.w_hidden_sizes,
        gamma=args.gamma,
        tau=args.tau,
