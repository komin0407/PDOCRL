...
from examples.configs.pdocrl_configs import PDOCRL_DEFAULT_CONFIG, PDOCRLTrainConfig
...
@pyrallis.wrap()
def train(args: PDOCRLTrainConfig):
    # update config: merge CLI overrides into task-specific defaults
    cfg, old_cfg = asdict(args), asdict(PDOCRLTrainConfig())
    differing_values = {key: cfg[key] for key in cfg.keys() if cfg[key] != old_cfg[key]}
    cfg = asdict(PDOCRL_DEFAULT_CONFIG[args.task]())
    cfg.update(differing_values)
    args = types.SimpleNamespace(**cfg)

    # setup logger
    default_cfg = asdict(PDOCRL_DEFAULT_CONFIG[args.task]())
